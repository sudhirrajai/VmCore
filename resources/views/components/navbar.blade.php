<nav class="flex items-center justify-between px-6 py-6 max-w-[1440px] mx-auto w-full bg-transparent">
  ...
  <div class="hidden md:flex items-center space-x-8 text-sm font-medium">
    ...
    <a href="{{ route('services') }}" class="transition-all {{ request()->routeIs('services', 'service.detail') ? 'border-b-2 border-black pb-1 text-slate-900' : 'text-slate-800 hover:opacity-60 transition-opacity' }}">Services</a>
    <a href="{{ route('portfolio') }}" class="transition-all {{ request()->routeIs('portfolio', 'portfolio.detail') ? 'border-b-2 border-black pb-1 text-slate-900' : 'text-slate-800 hover:opacity-60 transition-opacity' }}">Portfolio</a>
