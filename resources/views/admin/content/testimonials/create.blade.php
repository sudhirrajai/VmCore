            ...
            <div class="card-body">
                <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4 mb-3"><label class="form-label">Project</label><select
                                        ...
                                        <option value="">None</option>@foreach($projects as $p)<option value="{{ $p->id }}"
                                            {{ old('project_id') == $p->id ? 'selected' : '' }}>{{ $p->title }}</option>
                                        @endforeach
                                    </select></div>
                                <div class="col-md-4 mb-3"><label class="form-label">Team Member</label><select
                                        ...
                                        <option value="">None</option>@foreach($teamMembers as $m)<option
                                            value="{{ $m->id }}" {{ old('team_member_id') == $m->id ? 'selected' : '' }}>
                                        {{ $m->name }}</option>@endforeach
                                    </select></div>
                            </div>
                        </div>
                        <div class="col-md-4">
                    ...
                    <div class="mt-3"><button type="submit" class="btn btn-primary"><i class="bx bx-save me-1"></i>
                            Save</button> <a href="{{ route('admin.testimonials.index') }}"
